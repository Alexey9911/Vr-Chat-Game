...
const ROOM_LABELS: { label: string; position: [number, number, number]; fontSize?: number }[] = [
  { label: 'PumpFun Support',       position: [-40.81, 336.58, 117.70] },
  { label: 'Jewish Room',           position: [-77.39, 336.58,  67.68] },
  { label: 'PISS ON ME I AM A TOILET', position: [-104.50, 338.18, 108.94] },
  { label: 'Only Staff',            position: [-84.69, 338.18, 131.60] },
  { label: 'Woof Woof',             position: [ 69.60, 357.47,  34.80] },
  { label: 'HELP!!',                position: [ 82.32, 323.52, 117.73] },
]

function RoomLabel({ label, position, fontSize = 3 }: { label: string; position: [number, number, number]; fontSize?: number }) {
